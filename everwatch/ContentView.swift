...
struct WebsiteDetailView: View {
    ...
    var body: some View {
        VStack {
            ...
            Spacer() // Push the history list to the bottom
            ...
            List {
                ForEach(groupStatusPeriods(website.history)) { period in
                    HStack {
                        Text(period.status)
                        Spacer()
                        Text("\(period.startTime, style: .time) - \(period.endTime?.formatted(date: .omitted, time: .shortened) ?? "Now") (\(period.duration))") // Correct the ternary operator
                    }
...
struct AddWebsiteView: View {
    ...
    var body: some View {
        Form {
            ...
            Button("Add Website") {
                websites.append(Website(name: name, url: url))
                name = ""
                url = ""
            }
        }
...
func checkWebsiteStatus(urlString: String) async -> (Int?, String?, TimeInterval?) {
    ...
    // Load websites from UserDefaults to compare the previous status
    ...
    let isCritical = statusCode == nil || (statusCode! >= 300 && statusCode! <= 599)


       // Send notification only if the status has newly changed to critical
       if let previousStatus = websites.first(where: { $0.url == urlString })?.status,
          statusDescription != previousStatus,
          isCritical {
               sendNotification(title: "Website Status Changed", body: "\(urlString) is now \(statusDescription ?? "Unknown")", isCritical: isCritical)
